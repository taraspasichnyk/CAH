// This file is auto-generated. Do not edit manually!

import SwiftUI

// MARK: - Images

extension Image {
    static var back: Image { .init("back") }
    static var copy: Image { .init("copy") }
    static var four: Image { .init("four") }
    static var grid: Image { .init("grid") }
    static var host: Image { .init("host") }
    static var logo: Image { .init("logo") }
    static var one: Image { .init("one") }
    static var pattern: Image { .init("pattern") }
    static var share: Image { .init("share") }
    static var three: Image { .init("three") }
    static var two: Image { .init("two") }
}

// MARK: - Colors

extension Color {
    static var accentGrey: Color { .init("accentGrey") }
    static var mainBlack: Color { .init("mainBlack") }
    static var pendingYellow: Color { .init("pendingYellow") }
    static var readyGreen: Color { .init("readyGreen") }
}
