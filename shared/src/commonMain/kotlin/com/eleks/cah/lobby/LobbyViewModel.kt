...
class LobbyViewModel : BaseViewModel<LobbyContract.State, LobbyContract.Effect>(
    ...
    private fun enterName() {
        scope.launch {
            ...
            if (gameOwner) {
                val createdRoom = createRoomUseCase(state.value.name)
                ...
                player = joinRoomUseCase(state.value.code, state.value.name)
                setState {
    ...
    private fun enterCode() {
        scope.launch {
            setState { copy(isLoading = true) }
            val roomExist = roomExistUseCase(state.value.code)
    ...
    private fun subscribeOnUsers() {
        scope.launch {
            getRoomUseCase(state.value.code).filterNotNull().collect {
                setState {
                    if (gameOwner) {
